...
export default function MusicPage() {
  ...
  return (
    <main className={cn("min-h-screen px-4 sm:px-6 md:px-8 lg:p-12", currentIsDark ? "bg-slate-900 text-slate-100" : "bg-slate-50 text-slate-800")}>
      <motion.h1 
      ...
      <section>
        <motion.h2 
          className="text-3xl sm:text-4xl font-semibold mb-8 text-center sm:text-left [text-shadow:_0_1px_2px_rgba(0,0,0,0.05)] dark:[text-shadow:_0_1px_2px_rgba(0,0,0,0.1)]"
          variants={fadeInUp} initial="initial" animate="animate" transition={{ delay: 0.3 }} // Adjusted delay
        >
          Recommended Songs
        </motion.h2>
        <motion.div 
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8 w-full" // Added xl:grid-cols-4 and adjusted gap
          variants={staggerContainer(0.1, 0.4)} // Adjusted delay
          initial="initial"
          animate="animate"
        >
          {songs.map((song) => (
            <motion.div 
              ...
              variants={fadeInUp}
              className={cn(
                "rounded-xl shadow-lg overflow-hidden h-full flex flex-col", // Ensure flex column
                 currentIsDark ? "bg-slate-800 ring-1 ring-slate-700 hover:ring-sky-500" : "bg-white ring-1 ring-slate-200 hover:ring-sky-600",
            ...
            >
              <div className="relative w-full h-40 sm:h-48">
                <Image
                  ...
                  fill
                  className="object-cover transition-transform duration-300 ease-out hover:scale-105" // Subtle image zoom
                  sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
                />
              </div>
              <div className="p-5 flex-grow flex flex-col"> {/* Adjusted padding and flex */}
                <h3 className="text-xl font-bold mb-1 tracking-tight truncate hover:text-sky-500 dark:hover:text-sky-400 transition-colors" title={song.title}>{song.title}</h3>
                <p className={cn("text-sm flex-grow", currentIsDark ? "text-slate-400" : "text-slate-600")}>
                  By {song.artist}
                </p>
              </div>
